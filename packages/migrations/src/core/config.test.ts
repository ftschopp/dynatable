...
describe('ConfigLoader.load - validation', () => {
  ...
  test("rejects when tableName contains characters DynamoDB doesn't allow", async () => {
    ...
    await expect(new ConfigLoader(p).load()).rejects.toThrow(/tableName.*must be/i);
  });
  ...
  test('accepts valid tableName with underscores, dashes, dots, digits', async () => {
    ...
    const cfg = await new ConfigLoader(p).load();
  ...
  test('rejects when client.region is missing', async () => {
    ...
    await expect(new ConfigLoader(p).load()).rejects.toThrow(/region/);
  });
  ...
  test('rejects when migrationsDir points at an existing non-directory', async () => {
    ...
    await expect(new ConfigLoader(cfgPath).load()).rejects.toThrow(/not a directory/);
  });
